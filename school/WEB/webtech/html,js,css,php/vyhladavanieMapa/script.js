document.querySelectorAll('#countryVisitors tr').forEach(tr => tr.onclick = e => {
    document.querySelector('#visitorsFrom').classList.remove('hidden');

    const state = e.target.closest('tr').children[1].innerText;
    fetch(`stateHandler.php?state=${state}`).then(res => res.json()).then(data => {
        
        const tbody = document.querySelector('#add');
        tbody.innerHTML = '';
